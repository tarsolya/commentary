# Generated by jeweler
# DO NOT EDIT THIS FILE DIRECTLY
# Instead, edit Jeweler::Tasks in Rakefile, and run 'rake gemspec'
# -*- encoding: utf-8 -*-

Gem::Specification.new do |s|
  s.name = %q{commentary}
  s.version = "0.0.0"

  s.required_rubygems_version = Gem::Requirement.new(">= 0") if s.respond_to? :required_rubygems_version=
  s.authors = [%q{András Tarsoly}]
  s.date = %q{2011-09-01}
  s.description = %q{A simple gem for managing comments on ActiveRecord models.}
  s.email = %q{tarsolya@gmail.com}
  s.extra_rdoc_files = [
    "LICENSE.txt",
    "README.rdoc"
  ]
  s.files = [
    ".document",
    "Gemfile",
    "Gemfile.lock",
    "LICENSE.txt",
    "README.rdoc",
    "Rakefile",
    "VERSION",
    "commentary.gemspec",
    "init.rb",
    "install.rb",
    "lib/comment_methods.rb",
    "lib/commentable_methods.rb",
    "lib/commentary.rb",
    "lib/generators/USAGE",
    "lib/generators/commentary_generator.rb",
    "lib/generators/templates/comment.rb",
    "lib/generators/templates/comment_rating.rb",
    "lib/generators/templates/create_commentary.rb",
    "lib/rating_methods.rb",
    "test/commentary_generator_test.rb",
    "test/commentary_test.rb",
    "test/helper.rb",
    "test/test_commentary.rb",
    "test/test_helper.rb",
    "uninstall.rb"
  ]
  s.homepage = %q{http://github.com/tarsolya/commentary}
  s.licenses = [%q{MIT}]
  s.require_paths = [%q{lib}]
  s.rubygems_version = %q{1.8.7}
  s.summary = %q{Comments on ActiveRecord models.}

  if s.respond_to? :specification_version then
    s.specification_version = 3

    if Gem::Version.new(Gem::VERSION) >= Gem::Version.new('1.2.0') then
      s.add_runtime_dependency(%q<bundler>, ["~> 1.0.0"])
      s.add_runtime_dependency(%q<jeweler>, ["~> 1.6.4"])
      s.add_runtime_dependency(%q<rails>, ["= 3.1.0"])
      s.add_runtime_dependency(%q<mysql2>, [">= 0.2.4"])
    else
      s.add_dependency(%q<bundler>, ["~> 1.0.0"])
      s.add_dependency(%q<jeweler>, ["~> 1.6.4"])
      s.add_dependency(%q<rails>, ["= 3.1.0"])
      s.add_dependency(%q<mysql2>, [">= 0.2.4"])
    end
  else
    s.add_dependency(%q<bundler>, ["~> 1.0.0"])
    s.add_dependency(%q<jeweler>, ["~> 1.6.4"])
    s.add_dependency(%q<rails>, ["= 3.1.0"])
    s.add_dependency(%q<mysql2>, [">= 0.2.4"])
  end
end

